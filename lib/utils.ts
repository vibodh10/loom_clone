...
export const getMediaStreams = async (
    ...
    if (withMic) {
        try {
            ...
            console.error("Error getting microphone media:", err);
            micStream = null; // fallback gracefully if mic fails
        }
    }
...
export const setupRecording = (
    ...
    const config = mimeType ? { mimeType } : DEFAULT_RECORDING_CONFIG;
...
export const createIframeLink = (videoId: string) =>
    `https://iframe.mediadelivery.net/embed/428596/${videoId}?autoplay=true&preload=true`;
